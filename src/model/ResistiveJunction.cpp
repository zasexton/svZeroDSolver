...
void ResistiveJunction::update_constant(SparseSystem& system,
  ...
  for (size_t i = 0; i < num_inlets; i++) {
    system.add_F(global_eqn_ids[i], global_var_ids[i * 2], 1.0);
    system.add_F(global_eqn_ids[i], global_var_ids[i * 2 + 1],
                 -parameters[global_param_ids[i]]);
    system.add_F(global_eqn_ids[i], global_var_ids.back(), -1.0);
  }
  ...
  for (size_t i = 1; i < num_inlets * 2; i = i + 2) {
    system.add_F(global_eqn_ids[num_inlets + num_outlets],
